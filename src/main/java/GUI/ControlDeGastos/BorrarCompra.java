...
public class BorrarCompra extends javax.swing.JPanel {
    ...
    private void setupEnterKeyAction() {
        ...
        this.getActionMap().put("borrarCompra", new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) {
                borrarCompra();
            }
    ...
    private void borrarCompra() {
        ...
        Long compraId = Long.valueOf(id.getText());

        // Enviar una solicitud HTTP DELETE al servidor para eliminar la compra
        try {
            ...
            if (response.statusCode() == 200) {
                JOptionPane.showMessageDialog(this, "La compra fue eliminada con éxito");
                volverAtras();
            } else {
                JOptionPane.showMessageDialog(this, "Error al eliminar la compra: " + response.body());
            }
        } catch (Exception ex) {
            ex.printStackTrace();
